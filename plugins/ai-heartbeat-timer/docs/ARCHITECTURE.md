...
| `.ai-heartbeat-timer/` | Runtime-Folder (wird beim ersten Tick angelegt) | Runtime |
| `.ai-heartbeat-timer/state.json` | JSON-State (siehe unten) | Runtime |
| `.ai-heartbeat-timer/heartbeat.log` | Append-Log in lokaler Zeit | Runtime |
| `.ai-heartbeat-timer/heartbeat.lock` | Single-Firer Lease (siehe `lock.ts`) | Runtime |
| `.ai-heartbeat-timer/status-<instanceId>.txt` | Live-Status pro Instanz (overwrite, eine Datei pro laufendem Prozess) | Runtime |
...
| `mcp/heartbeat-channel/state.ts` | CLI State-Ops: `show`, `delete NAME`, `rename OLD NEW`, `touch NAME`. Wird vom `ai-timer` Skill benutzt damit State konsistent mit timer.md bleibt. | -- |
...
## State-Format (`.ai-heartbeat-timer/state.json`)
...
| `.ai-heartbeat-timer/state.json` fehlt | Fresh state, alle Timer koennen heute noch feuern |
| `.ai-heartbeat-timer/state.json` korrupt | WARN + reset, wie "fehlt" |
| `.ai-heartbeat-timer/` fehlt | Wird beim ersten Tick via `mkdirSync(recursive)` angelegt |
...
#    e. cat .ai-heartbeat-timer/state.json -> Name in firedToday oder lastFire
#    f. tail -5 .ai-heartbeat-timer/heartbeat.log -> FIRED-Zeile
...
| Symptom | Check |
|---------|-------|
| Timer feuert nicht | `tail -f .ai-heartbeat-timer/heartbeat.log` + `cat .ai-heartbeat-timer/state.json`. Ist Name in `firedToday`? In `errors`-Report? |
...
| **V3.2** | + State-CLI (`state.ts`), + DST-Clamp in `intervalDue`, + `ai-timer` Skill mit DELETE/UPDATE inkl. State-Migration | Kollisionsfreier Timer-Lifecycle |
