#!/usr/bin/env bash
# SessionStart hook for ai-heartbeat-timer.
...
TIMER="$PROJ/timer.md"
LOG="$PROJ/.ai-heartbeat-timer/heartbeat.log"
...
build_message() {
  echo "=== ai-heartbeat-timer: timer.md ==="
  cat "$TIMER"
  echo
  echo "=== ai-heartbeat-timer: channel status ==="
  ...
  if [ ! -f "$LOG" ]; then
    cat <<'INACTIVE'
Status: INACTIVE -- .ai-heartbeat-timer/heartbeat.log fehlt im Projektroot.
...
werden NICHT feuern. Beende diese Session und starte Claude mit:
  ...
  claude --dangerously-load-development-channels plugin:ai-heartbeat-timer@ai-plugins --dangerously-skip-permissions
...
INACTIVE
  ...
  if [ -z "$last_epoch" ]; then
    echo "Status: UNKNOWN -- .ai-heartbeat-timer/heartbeat.log existiert, aber letzte Zeile nicht parsbar:"
    echo "  $last_line"
    return 0
  fi
