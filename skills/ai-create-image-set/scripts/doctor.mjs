#!/usr/bin/env node
// ai-create-image-set doctor -- read-only Healthcheck (schreibt keine Marker).
...
const isPlaceholder = (k) => !k || /^(REPLACE_ME|TODO|XXX|<.*>)/i.test(k) || k.length < 10
{
  ...
  else if (d && isPlaceholder(d)) warn('OPENAI_API_KEY', `Default-Key ist Platzhalter -- ~/.claude/.env/ai-create-image-set.env anlegen`)
...
{
  try {
    const dir = mkdtempSync(path.join(tmpdir(), 'ai-create-image-set-doctor-'))
...
// 4. Wrapper present
{
  const wrapper = path.join(SKILL_DIR, 'bin', 'create-image-set.mjs')
...
console.log()
console.log(`ai-create-image-set doctor (Plattform: ${platform()})`)
