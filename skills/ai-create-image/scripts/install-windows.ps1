# ai-create-image -- One-Shot-Installer fuer Windows
# Kopiert das Skill nach %USERPROFILE%\.claude\skills\ai-create-image\ und startet den Preflight.
#
...
$Target      = Join-Path $ClaudeRoot 'ai-create-image'

function Step($m) { Write-Host "`n==> $m" -ForegroundColor Cyan }
function Ok($m)   { Write-Host "    OK   $m" -ForegroundColor Green }
function Warn($m) { Write-Host "    WARN $m" -ForegroundColor Yellow }

Step "ai-create-image One-Shot-Installer (Windows)"
...
Write-Host ""
if ($pfExit -eq 0) {
    Ok "Fertig. Skill 'ai-create-image' ist in Claude Code einsatzbereit."
