#!/usr/bin/env node
// ai-create-image preflight core
...
const SKILL_NAME = 'ai-create-image'
...
function checkKeyPresent() {
  ...
  if (!auth) {
    ...
    record('OPENAI_API_KEY (Konfig)', 'fail',
      `Loesung A: \`mkdir -p ~/.claude/.env && echo "OPENAI_API_KEY=sk-..." > ~/.claude/.env/ai-create-image.env\`\n\n` +
...
async function checkKeyLive(auth) {
  ...
  if (res.status === 401) {
    ...
    record('OPENAI_API_KEY (live)', 'fail', `HTTP 401 -- Key ungueltig oder abgelaufen. Neuen Key in ~/.claude/.env/ai-create-image.env eintragen.`)
    return false
  }
  if (!res.ok) {
