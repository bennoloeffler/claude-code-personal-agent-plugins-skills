...
/plugin install heartbeat-timer@<marketplace>
```

> Replace `<owner>/<marketplace-repo>` with the GitHub repo that hosts this plugin's `.claude-plugin/marketplace.json`, and `<marketplace>` with the marketplace name registered there. (Throughout this README, `MARKETPLACE_NAME` is used as a placeholder — substitute your real marketplace name.)
...
```bash
claude --dangerously-load-development-channels plugin:heartbeat-timer@MARKETPLACE_NAME
```
...
```bash
claude --channels plugin:heartbeat-timer@MARKETPLACE_NAME
```
...
```json
{
  ...
  "allowedChannelPlugins": [
    { "plugin": "heartbeat-timer", "marketplace": "MARKETPLACE_NAME" }
  ]
}
```
...
After launching with the flag, the session's system reminders include a block starting with `## plugin:heartbeat-timer:heartbeat` and describing how channel events arrive. If you don't see that, the channel is still gated — re-check the gates above.

When a timer fires, you'll get a `<channel source="heartbeat" name="..." time="...">PROMPT</channel>` event in your session, and a `.heartbeat-timer/` folder (containing `heartbeat.log`, `state.json`, `heartbeat.lock`, `status-<id>.txt`) appears in the project root.

## Quickstart

1. Create `timer.md` in your project root (or copy the template):
   ```bash
   cp "$(claude plugin root heartbeat-timer)/templates/timer.md" ./timer.md
   ```
...
| Symptom | Cause | Fix |
|---|---|---|
| Timer never fires, no `.heartbeat-timer/heartbeat.log` | Channel flag missing | Re-launch with `--dangerously-load-development-channels plugin:heartbeat-timer@MARKETPLACE_NAME` |
| `.heartbeat-timer/heartbeat.log` shows `FIRED` but no event in session | OAuth missing (gate 3) | `/login` to claude.ai |
...
| "server X not in --channels list for this session" | Forgot to name the plugin on launch | Add `plugin:heartbeat-timer@MARKETPLACE_NAME` after the flag |
| Timer fires twice | State file out of sync after rename | `bun "$(claude plugin root heartbeat-timer)/mcp/heartbeat-channel/state.ts" show` to inspect; the `/timer` skill auto-migrates state on rename |
...
4. Tracks state in `.heartbeat-timer/state.json` (`firedToday`, `lastFire`)
5. Logs to `.heartbeat-timer/heartbeat.log` (local time)
...
`timer.md` lives at your project root; all runtime files (log, state, lock, per-instance status) live in `.heartbeat-timer/` so they don't pollute the top level. The plugin code itself stays in Claude's plugin cache.

## Use the timer skill to manage timers

The plugin bundles a `/timer` skill that adds/changes/removes timers via natural language (English + German trigger phrases):

```
/timer every weekday at 8am ask me what's coming up today
/timer every 2h ask me about pause
/timer change morning-check to 9:00
/timer delete pause-nudge
```
...
| `.heartbeat-timer/heartbeat.log` | Runtime log (append-only) | ❌ (gitignore the whole folder) |
| `.heartbeat-timer/state.json` | `firedToday` + `lastFire` state | ❌ |
| `.heartbeat-timer/heartbeat.lock` | Single-firer lock | ❌ |
| `.heartbeat-timer/status-<id>.txt` | Per-instance live status (one per running session) | ❌ |

Add `.heartbeat-timer/` to your project's `.gitignore`.

## Manual ops

```bash
# Validate timer.md
bun "$(claude plugin root heartbeat-timer)/mcp/heartbeat-channel/validate.ts"

# State operations
bun "$(claude plugin root heartbeat-timer)/mcp/heartbeat-channel/state.ts" show
bun "$(claude plugin root heartbeat-timer)/mcp/heartbeat-channel/state.ts" delete morning-todos
bun "$(claude plugin root heartbeat-timer)/mcp/heartbeat-channel/state.ts" rename old-name new-name
```
