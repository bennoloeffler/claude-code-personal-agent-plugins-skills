...
| Signal | Wert |
|--------|------|
| `.heartbeat-timer/heartbeat.log` existiert? | ja/nein |
| `.heartbeat-timer/state.json` existiert? | ja/nein |
| `timer.md` existiert? | ja/nein |
| System-Reminder-Block `## plugin:heartbeat-timer:heartbeat` in dieser Session? | ja/nein (Indikator dass Channel-Flag gesetzt ist) |

**Onboarding-Modus** ausloesen wenn ALLE drei zutreffen:
- `.heartbeat-timer/heartbeat.log` fehlt **und**
- `.heartbeat-timer/state.json` fehlt **und**
...
Session ein Block `## plugin:heartbeat-timer:heartbeat` auftauchen. Wenn ja:
Channel ist aktiv, Timer feuern.

Volle Doku + Troubleshooting:
   $(claude plugin root heartbeat-timer)/README.md
...
Der Runtime matched State per Name. Wenn sich der Name oder das Schedule-Kind aendert, muss `.heartbeat-timer/state.json` nachgezogen werden -- sonst gibt's ueberraschende Doppel-Fires.
...
### Beispiel 1: Einfach
**User**: `/timer jeden werktag 9 uhr check emails`
...
**User**: `/timer alle 2h mich fragen ob ich pause mache`
...
**User**: `/timer morgens, mittags, abends kalender check`
...
**User**: `/timer jeden morgen 7 uhr tagescheck` aber `morgen-check` existiert schon mit 07:00 weekdays.
